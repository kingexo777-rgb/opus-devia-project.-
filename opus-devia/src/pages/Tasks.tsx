...
function getBadgeGlow(level: number): string {
  if (level >= 100)
    return "0 0 60px rgba(255,69,0,0.7), 0 0 120px rgba(255,0,0,0.4)";

  if (level >= 80)
    return "0 0 45px rgba(224,176,255,0.6), 0 0 90px rgba(128,0,128,0.3)";

  if (level >= 70)
    return "0 0 35px rgba(224,176,255,0.5), 0 0 70px rgba(128,0,128,0.25)";

  if (level >= 50)
    return "0 0 30px rgba(255,215,0,0.5), 0 0 60px rgba(184,134,11,0.3)";

  if (level >= 40)
    return "0 0 25px rgba(255,215,0,0.4), 0 0 50px rgba(184,134,11,0.2)";

  if (level >= 30)
    return "0 0 20px rgba(192,192,192,0.4), 0 0 40px rgba(112,112,112,0.2)";

  if (level >= 20)
    return "0 0 18px rgba(192,192,192,0.35), 0 0 35px rgba(112,112,112,0.18)";

  if (level >= 10)
    return "0 0 15px rgba(205,127,50,0.35), 0 0 30px rgba(139,69,19,0.18)";
...
async function fetchPriorityTask(
  ...
  // 3. Get first incomplete task in that phase
  const { data: tasks, error: tasksErr } = await supabase
    .from("tasks")
    .select(
      "id, title, description, estimated_hours, xp_reward, status, order_index",
    )
...
export default function Tasks() {
   ...
   * 1. profile.username
   * 2. profile.full_name
   * 3. Supabase username metadata
   * 4. Supabase full_name metadata
   * 5. Email username
   * 6. "User" as final fallback
   */
  const displayName =
  ...
  const [priorityTask, setPriorityTask] =
    useState<PriorityTaskData | null>(null);
  const [loading, setLoading] = useState(true);
  const [startingTask, setStartingTask] = useState(false);

  /* ---------------------------------------------------------------- */
  /*  Fetch XP + Priority Task                                        */
  /* ---------------------------------------------------------------- */
    ...
    <div style={styles.container}>
      ...
      >
        <button
          style={{
            ...
            borderRadius: 999,
            border:
              "1px solid var(--glossy-pill-border, var(--theme-accent, rgba(255,59,48,0.5)))",
            cursor: "pointer",
            background:
              "var(--glossy-pill-bg, var(--pill-bg, linear-gradient(135deg, rgba(120,10,10,0.9) 0%, rgba(40,4,4,0.95) 100%)))",
            boxShadow:
              "var(--glossy-pill-shadow, 0 0 20px var(--theme-accent-glow, rgba(200,0,0,0.3)), 0 0 40px var(--theme-accent-glow, rgba(200,0,0,0.15)), 0 4px 12px rgba(0,0,0,0.4))",
            display: "flex",
            alignItems: "center",
            gap: 10,
            overflow: "hidden",
      ...
      <style>{`
        @keyframes chatPillPulse {
          0%, 100% {
            box-shadow:
              0 0 20px var(--theme-accent-glow, rgba(200,0,0,0.3)),
              0 0 40px var(--theme-accent-glow, rgba(200,0,0,0.15)),
              0 4px 12px rgba(0,0,0,0.4);
          }

          50% {
            box-shadow:
              0 0 30px var(--theme-accent-glow, rgba(200,0,0,0.5)),
              0 0 60px var(--theme-accent-glow, rgba(200,0,0,0.25)),
              0 4px 12px rgba(0,0,0,0.4);
          }
        }
