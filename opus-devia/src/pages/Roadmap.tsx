...
function getDifficultyColor(
  ...
  if (d === "large") {
    return {
      bg: "color-mix(in srgb, var(--theme-accent, #DC143C) 12%, transparent)",
      text: "var(--theme-accent, #DC143C)",
      border:
        "color-mix(in srgb, var(--theme-accent, #DC143C) 35%, transparent)",
      label: "Hard",
    };
  }
  ...
  if (d === "small") {
    return {
      bg: "rgba(135, 206, 250, 0.12)",
      text: "#87CEEB",
      border: "rgba(135, 206, 250, 0.35)",
      label: "Easy",
    };
  }
...
export default function Roadmap() {
  ...
  const fetchRoadmapRef = useRef<() => Promise<void>>(
    () => Promise.resolve()
  );
  ...
  const fetchRoadmap = useCallback(async () => {
    ...
    try {
      /* ---- 1. Get most recent active roadmap ---- */
      ...
      const { data: phases, error: phasesErr } = await supabase
        .from("roadmap_phases")
        .select(
          "id, title, description, phase_order, is_completed, status, tasks!tasks_roadmap_phase_id_fkey(*)"
        )
  ...
  useEffect(() => {
    if (
      !roadmap ||
      !initialTaskId ||
      selectedTask?.task.id === initialTaskId
    ) {
      return;
    }
    ...
    async (taskId: string, phaseId: string) => {
      ...
      const updatedPhases = roadmap.phases.map((p) => {
        ...
        const allDone =
          ...
          updatedTasks.every((t) => t.is_completed);
      ...
      const phaseTasks =
        updatedPhases.find((p) => p.id === phaseId)?.tasks ?? [];

      if (
        phaseTasks.length > 0 &&
        phaseTasks.every((t) => t.is_completed)
      ) {
    ...
    <div className="roadmap-page">
      ...
      <ul className="roadmap-timeline">
        {phases.map((phase, index) => {
          ...
          if (locked) {
            nodeClass += " locked-phase";
          }

          if (showUnlockAnimation === phase.id) {
            nodeClass += " unlocking";
          }

          const circleClass = `roadmap-node-circle ${
            phase.is_completed
              ? "completed"
              : locked
              ? "locked"
          ...
          const isExpanded = expandedPhases.has(
            phase.id
          );
            ...
            >
              ...
              >
                {locked ? (
                  ...
                  <div
                    className="roadmap-node-btn locked-card"
                    title={
                      phase.description ??
      ...
      <style>{`
        ...
        @keyframes unlockGlowPulse {
          ...
          50% {
            box-shadow:
              0 0 20px
              var(--theme-accent-glow, rgba(154,0,0,0.6));
          }
        ...
        .roadmap-node.unlocking
          .roadmap-node-body.body-locked {
          animation:
            unlockShake 400ms ease-out,
            unlockGlowPulse 800ms ease-out 400ms;
        ...
        .roadmap-node.unlocking
          .locked-card {
          animation:
            unlockTitleFade 300ms ease-out forwards;
        }

        .roadmap-node.unlocking
          .roadmap-lock-icon {
          animation:
            lockIconDissolve 300ms ease-out forwards;
        }
        ...
        <div className="phase-complete-overlay">
          <div className="phase-complete-content">
            ...
            <svg
              ...
              viewBox="0 0 24 24"
              fill="none"
              stroke="var(--theme-accent, #9a0000)"
              strokeWidth="2"
              strokeLinecap="round"
              strokeLinejoin="round"
              style={{
                ...
                strokeDashoffset: 60,
                animation:
                  "phaseCompleteCheckDraw 600ms ease-out forwards",
              }}
            >
            ...
            <h2
              style={{
                ...
                opacity: 0,
                animation:
                  "phaseCompleteFadeIn 500ms ease-out 400ms forwards",
              }}
            >
              Phase Complete
            </h2>
            ...
            <p
              style={{
                ...
                opacity: 0,
                animation:
                  "phaseCompleteFadeIn 500ms ease-out 600ms forwards",
              }}
            >
              +
              {roadmap.phases
                .find((p) => p.is_completed)
                ?.tasks.reduce(
                  (s, t) => s + (t.xp_reward ?? 0),
                  0
                ) ?? 0}{" "}
      ...
      <Link
        to="/mentor"
        className="roadmap-floating-mentor"
  ...
  return (
    <div className="roadmap-header">
      ...
      <div className="roadmap-header-left">
        <span className="roadmap-username">
          {displayName}
        </span>
      ...
      <Link
        to="/profile"
        className="roadmap-settings-btn"
        aria-label="Settings"
      >
        <svg
          width="18"
...
function TaskDetailModal({
  ...
  const dc = getDifficultyColor(
    task.difficulty
  );

  const goalText =
    task.goal_description?.trim() ||
    "No difficulty breakdown available.";

  const rationaleText =
    task.difficulty_rationale?.trim() ||
    "No difficulty breakdown available.";
  ...
  return (
    ...
    >
      ...
      >
        ...
        {phaseTitle && (
          <div className="task-modal-synthetic-tag">
            {phaseTitle}
          </div>
        )}
        ...
        <div className="task-modal-meta">
          ...
          >
            ...
            {task.difficulty_score != null
              ? ` · ${task.difficulty_score}/5`
              : ""}
          </span>

          {typeLabel && (
            <span className="task-modal-badge task-type-badge">
              {typeLabel}
            </span>
          )}
          ...
          {task.is_completed && (
            <span className="task-modal-badge task-done-badge">
              Completed
            </span>
          )}
